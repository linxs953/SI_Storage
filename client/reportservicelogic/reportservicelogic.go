...
type (
	...
	defaultReportServiceLogic struct {
		cli zrpc.Client
	}
)

func NewReportServiceLogic(cli zrpc.Client) ReportServiceLogic {
	return &defaultReportServiceLogic{
		cli: cli,
	}
}

// 测试报告
func (m *defaultReportServiceLogic) GetReport(ctx context.Context, in *GetTestReportRequest, opts ...grpc.CallOption) (*TestReportResponse, error) {
	client := storage.NewReportServiceLogicClient(m.cli.Conn())
	return client.GetReport(ctx, in, opts...)
}

func (m *defaultReportServiceLogic) ListReports(ctx context.Context, in *GetTaskReportListRequest, opts ...grpc.CallOption) (*ReportListResponse, error) {
	client := storage.NewReportServiceLogicClient(m.cli.Conn())
	return client.ListReports(ctx, in, opts...)
}

func (m *defaultReportServiceLogic) DeleteReport(ctx context.Context, in *GetTestReportRequest, opts ...grpc.CallOption) (*DeleteResponse, error) {
	client := storage.NewReportServiceLogicClient(m.cli.Conn())
