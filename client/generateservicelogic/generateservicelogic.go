...
type (
	...
	GenerateServiceLogic interface {
	...
	defaultGenerateServiceLogic struct {
		cli zrpc.Client
	}
)

func NewGenerateServiceLogic(cli zrpc.Client) GenerateServiceLogic {
	return &defaultGenerateServiceLogic{
		cli: cli,
	}
}

// 根据ApiInfo生成依赖、提取器、预期
func (m *defaultGenerateServiceLogic) GenerateDependency(ctx context.Context, in *GenerateDependencyRequest, opts ...grpc.CallOption) (*GenerateDependencyResponse, error) {
	client := storage.NewGenerateServiceLogicClient(m.cli.Conn())
	return client.GenerateDependency(ctx, in, opts...)
}

func (m *defaultGenerateServiceLogic) GenerateExtractor(ctx context.Context, in *GenerateExtractorRequest, opts ...grpc.CallOption) (*GenerateExtractorResponse, error) {
	client := storage.NewGenerateServiceLogicClient(m.cli.Conn())
	return client.GenerateExtractor(ctx, in, opts...)
}

func (m *defaultGenerateServiceLogic) GenerateExpect(ctx context.Context, in *GenerateExpectRequest, opts ...grpc.CallOption) (*GenerateExpectResponse, error) {
	client := storage.NewGenerateServiceLogicClient(m.cli.Conn())
