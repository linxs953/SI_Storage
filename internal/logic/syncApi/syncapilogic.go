...
func (l *SyncApiLogic) SyncApi(req *types.SyncDto) (resp *types.SyncVO, err error) {
	resp = &types.SyncVO{
		Code:    0,
		Message: "创建同步任务成功",
		Data:    nil,
	...
	if err = getApifoxAuth("2e301f8d-5ee8-4b0d-a111-0a21c30ba557", "psu123456"); err != nil {
		return nil, err
	}
	var body map[string]interface{}
	if body, err = getApiTree(); err != nil {
		return nil, err
	}
	var apiIds []string
	if apiIds, err = extractApiDetailIdList(body); err != nil {
		return nil, err
	}
	folderLevelMap, err := buildFolderLevelMap()
	body = make(map[string]interface{})
	body["folderLevel"] = folderLevelMap
	body["apiCounts"] = len(apiIds)
	if err != nil {
		return nil, err
	}

	recordId := l.ctx.Value("SyncRecordID").(primitive.ObjectID)
	go handleExtractTask(l.svcCtx, apiIds, folderLevelMap, recordId)
	resp.Data = body
	return
}

func handleExtractTask(ctx *svc.ServiceContext, apiIds []string, folderLevelMap map[string]string, recordId primitive.ObjectID) {
	var err error
	var apiDetailList []apidetail.Apidetail
	for _, apiId := range apiIds {
		apiDetail, err := extractApiInfo(apiId, folderLevelMap)
		if err != nil {
			logx.Error(err)
			continue
		}
		apiDetailList = append(apiDetailList, apiDetail)
	}

	if err = pushSyncMsg(ctx, apiDetailList, recordId); err != nil {
		logx.Error(err)
	}
}

func pushSyncMsg(ctx *svc.ServiceContext, apiDetailList []apidetail.Apidetail, updateId primitive.ObjectID) error {
	logx.Info("数据开始入库")
	var syncEvents []ApiSyncEvent
	for idx, apide := range apiDetailList {
		if apide.ID.IsZero() {
			apide.ID = primitive.NewObjectID()
		}
		apide.CreateAt = time.Now()
		apide.UpdateAt = time.Now()
		syncEvent := ApiSyncEvent{
			Type:     "sync_apidetail",
			Data:     apide,
			UpdateId: updateId,
		}
		if idx == len(apiDetailList)-1 {
			syncEvent.IsEof = true
		}
	...
	for _, event := range syncEvents {
		...
		if err != nil {
			logx.Error("序列化 apidetail 事件消息失败", err)
			return err
		}
...
func getApifoxAuth(user, passwd string) (err error) {
	var body []common.RequestFormBodyParameter
	body = append(body, common.RequestFormBodyParameter{FormName: "id", FormValue: user})
	body = append(body, common.RequestFormBodyParameter{FormName: "password", FormValue: passwd})
	...
	req := common.Request{
		Method:      "POST",
		ReqUrl:      "https://apifox.com/api/v1/shared-doc-auth",
...
func getApiTree() (map[string]interface{}, error) {
	...
	req := common.Request{
		Method:      "GET",
		ReqUrl:      "https://apifox.com/api/v1/shared-docs/2e301f8d-5ee8-4b0d-a111-0a21c30ba557/http-api-tree",
		SetCookies:  false,
		NeedCookies: true,
		Headers:     headers,
	}
	...
	_, err = json.Marshal(bodyMap)
	if err != nil {
		logx.Error("json序列化失败")
		return nil, err
	}
...
func extractApiDetailIdList(resp map[string]interface{}) (apiIdList []string, err error) {
	...
	for _, group := range findApiIdRegex.FindAllStringSubmatch(string(bts), -1) {
		if len(group) < 2 {
			continue
		}
		apiIdList = append(apiIdList, group[1])
	}
	return
}

func buildFolderLevelMap() (folderLevelMap map[string]string, err error) {
	resp, err := getApiTree()
	if err != nil {
		return
	}
...
func extractApiInfo(apiId string, foldersLevel map[string]string) (apiDetail apidetail.Apidetail, err error) {
	apiDetail.Source = logic.SYNC_APIFOX
	getDetailUrl := fmt.Sprintf("https://apifox.com/api/v1/shared-docs/2e301f8d-5ee8-4b0d-a111-0a21c30ba557/http-apis/%s?locale=zh-CN", apiId)
